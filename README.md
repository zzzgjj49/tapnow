...
已添加邀请登录、成员停用、个人生成用量、CSV 明细导出，以及按实际月度账单选择成员平分的功能。云端模式使用 PostgreSQL 保存项目和任务，浏览器分片直传私有 TOS，QStash 负责关闭页面后的查询与归档。受邀成员共用同一个项目空间；目前没有实时多人光标或编辑冲突合并。

配置 `DATABASE_URL` 后启用云端模式，强制登录；未配置时继续原有本地模式。已有项目可用 `npm run migrate:cloud` 迁移。具体步骤见 [Vercel 部署说明](docs/vercel-deployment.md)。数据库、队列与正式域名尚需在服务控制台配置，代码和本地测试完成不等于已经上线。
